...
func TestLiveAPIParity(t *testing.T) {
	...
	query := func(limit int, where string) any {
		spec := map[string]any{
			"name": "A", "signal": "logs", "offset": 0, "limit": limit,
			"order": []any{
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			status, raw := direct(t, "POST", "/api/v5/query_range", key, query(test.limit, test.where))
			if status != 200 {
	...
	t.Run("invalid-filter", func(t *testing.T) {
		status, _ := direct(t, "POST", "/api/v5/query_range", key, query(1, "severity_text ="))
